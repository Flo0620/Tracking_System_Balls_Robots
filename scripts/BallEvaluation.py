"""
This script can be used to evaluate the precision of the Ball Detection System. 
For that a target pose is created at a hand-measured position of the ball on the field.
 The measurements are taken in the coordinate system of the field middle.
Then 10 detected poses of the ball are compared with the target pose and the errors are computed and saved in a txt file
...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.DIR='BallEvaluation/EvaluationImages' #Directory where for each evaluated position one image is saved
        self.imgnumber = len([name for name in os.listdir(self.DIR) if os.path.isfile(os.path.join(self.DIR, name))])+1
        self.imageName = "evalImg00"+str(self.imgnumber)+'.png'
    ...
    def maketarget(self):
        ...
        static_transformStamped.header.frame_id = 'field'
        static_transformStamped.child_frame_id = 'targetPose'
        #measured values where the ball is with respect to the mid of the field
        #have to be changed for each position at which the ball tracking should be evaluated
        self.targetx=-2.0
        self.targety=1.0
        self.targetz=0.0725
    ...
    def listener_callback_Image(self, msg):
        ...
        if self.savedImage == False:
            ...
            cv2.imwrite(self.DIR+'/'+self.imageName,img)
    ...
    def TFlistener_callback(self, msg):
        ...
        try:
            ...
            trans = self.tfBuffer.lookup_transform('targetPose','ball0',rclpy.time.Time())
            transFT = self.tfBuffer.lookup_transform('field','targetPose',rclpy.time.Time())
            transFM = self.tfBuffer.lookup_transform('field','ball0',rclpy.time.Time())
            ...
            self.get_logger().info('I heard ball0')
            ...
            measurementPath = "BallEvaluation/EvaluationData/measurements.txt"
            if self.counterMeasurements==0 and self.imageName!='':
                f=open(measurementPath,'a')
